...
func (m *ioDeviceWeightPrometheusMetrics) register(registry prometheus.Registerer, namespace string, labels prometheus.Labels) {
	...
	m.values = promauto.With(registry).NewGaugeVec(prometheus.GaugeOpts{Namespace: namespace, Name: "io_device_weight_value", Help: "Exact requested, systemd-derived, and kernel-read-back weight by sibling and device", ConstLabels: labels}, []string{"uid", "class", "device", "mechanism", "coverage", "stage"})
	m.nominalShare = promauto.With(registry).NewGaugeVec(prometheus.GaugeOpts{Namespace: namespace, Name: "io_device_weight_nominal_share", Help: "Nominal requested share in the current sibling denominator by slice and device; this is not delivered throughput", ConstLabels: labels}, []string{"uid", "class", "device", "mechanism", "coverage"})
