# Upgrading from ResMan 1.25.x through 1.35.4 to ResMan 1.36.5

Current metrics schema: 7.

This guide applies when moving from any ResMan release from 1.25.x through 1.35.4 to
ResMan 1.36.5. This guide covers the post-1.25.1 audit remediation, the CPU Points
...
ResMan 1.36.5 validates the systemd-native
...
of running with an enforcement policy that the host cannot apply. On systemd 239 the
startup probe first asks systemd to materialize the I/O controller with its neutral
weight before it verifies `io.max`, then proves a non-default weight against either
the direct or scaled BFQ kernel representation. A real user slice may still lack
`io.max` until its first I/O property is applied; after complete workload-authority
verification, ResMan permits that one absent interface to be materialized by the
transactional systemd write and acknowledges it only after exact kernel readback.
...
failure with its path, separately from a missing controller or cgroup interface.
...
**Visible change.** The packaged `resman.service` still runs as root, but it now has
an explicit capability bounding set, read-only `/usr`, boot directories and `/etc`,
hidden home directories, private temporary directories, restricted namespace creation
and a limited address-family set. Limit hooks inherit the same mount namespace and
capability ceiling. A hook under `/home`, `/root` or `/run/user` is no longer visible;
its `/tmp` and `/var/tmp` are private; and it cannot write below `/etc` outside
`/etc/resman`.

**Cause.** Systemd authorizes unit-property mutation by the root identity through
polkit rather than by a Linux capability. Keeping the root identity while bounding
capabilities and file-system visibility reduces the privilege surface without
weakening systemd-native enforcement. `CAP_SYS_RESOURCE` remains present because PSI
poll-trigger registration requires it; the other retained capabilities each serve
cross-user observation, non-root hook execution, hook process-group termination or
privileged listener ports.

**Action.** Install hook programs under a system path such as `/usr/local/libexec`,
use `/etc/resman` for configuration writes, and do not use host `/tmp` as an exchange
