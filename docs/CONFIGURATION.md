...
|---|---|---|---|---|---|---|---|---|
| `AUTODETECT_PATTERNS` | `boolean` | `false` | `dynamic` | `false` | `true` | — | false disables observational workload-pattern classification. | Correct the authored value and submit the complete candidate for validation. |
...
| `IO_USER_EXCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty excludes nobody from I/O eligibility. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
| `IO_USER_INCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty includes every non-excluded user for I/O eligibility. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
...
| `PROCESS_EXCLUDE_LIST` | `string_list` | `^systemd$,^dbus-daemon$,^dbus-broker$,^polkitd$` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty excludes no process from enforcement. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
...
| `RAM_USER_EXCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty excludes nobody from RAM eligibility. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
| `RAM_USER_INCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty includes every non-excluded user for RAM eligibility. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
...
| `USER_EXCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty excludes nobody from CPU eligibility. Excluded native user slices remain inside the parent quota and share aggregate best effort. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
| `USER_INCLUDE_LIST` | `string_list` | `(empty)` | `dynamic` | `false` | `true` | format comma-separated-regex-list | Empty makes no user eligible for CPU Points enforcement; observation remains active. Use .* for every non-excluded user. | A comma is the list separator and cannot appear inside a regex pattern, including a bounded quantifier such as {2,4}; rewrite the expression without a comma, for example by expanding the alternatives. |
