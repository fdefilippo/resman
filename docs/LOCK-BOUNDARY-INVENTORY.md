...
| `internal/systemdunit.Adapter` | `internal/systemdunit.Adapter` | operation-owned lease map only | `opGate` | Discovery, runtime-only D-Bus mutation, readback, read-only cgroup verification and compare-before-restore are serialized by the gate; no state mutex is held across external I/O. |
