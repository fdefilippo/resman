...
| `internal/systemdunit.Adapter` | `internal/systemdunit.Adapter` | operation-owned lease and recovery maps only | `opGate` | Discovery, durable lease-journal replacement, runtime-only D-Bus mutation, readback, read-only cgroup verification and compare-before-restore are serialized by the gate; no state mutex is held across external I/O. |
