...
| `internal/systemdunit.Adapter` | `internal/systemdunit.Adapter` | operation-owned lease and recovery maps only | `opGate` | Discovery and exact topology confirmation, durable lease-journal replacement, runtime-only D-Bus mutation, readback, read-only cgroup verification, identity-stable accounting, CPU coverage scans and compare-before-restore are serialized by the gate; no state mutex is held across external I/O. |
