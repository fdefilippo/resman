	...
	tests := []struct {
		...
		},
		{
			...
			required: []string{
				...
				"sudo test -e $(CONF_DIR)/io-weights.map || sudo test -L $(CONF_DIR)/io-weights.map",
		...
		},
		{
			...
			required: []string{
				...
				"assert_entry '/etc/resman/io-weights.map' '-rw-------' 'root/root' \"$listing\"",
				...
				"assert_entry '/usr/share/doc/resman/SYSTEMD-PROPERTY-LEASES.md' '-rw-r--r--' 'root/root' \"$listing\"",
				"assert_entry '/usr/share/doc/resman/CPU-POINTS-OBSERVABILITY.md' '-rw-r--r--' 'root/root' \"$listing\"",
				"assert_entry '/usr/share/doc/resman/IO-WEIGHTS.md' '-rw-r--r--' 'root/root' \"$listing\"",
			},
		},
	}
