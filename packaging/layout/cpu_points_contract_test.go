...
func TestCPUPointsOperatorSurfacesCarryTheCompleteContract(t *testing.T) {
	...
	requiredByPath := map[string][]string{
		"README.md": {
			...
			"guaranteed_domain_usage_delta / parent_usage_delta",
		...
		"docs/ARCHITECTURE.md": {
			"mapped_leaf_usage_delta       / parent_usage_delta",
			"60-second window",
			"sum of acquired/applied mapped guarantees",
			"john.smith",
			"post-ingress",
		},
		"docs/TECHNICAL-SPECIFICATION.md": {
			"floor(online_cpus * period * (1000 - reserve) / 1000)",
			"minimum quota is 1000 microseconds",
			"mapped_leaf_usage_delta       / parent_usage_delta",
			"60-second window",
			"pending state",
		},
