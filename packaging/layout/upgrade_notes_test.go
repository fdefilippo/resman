...
func TestUpgradeGuideIsReferencedAndPackaged(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path:     "README.md",
			required: []string{"docs/UPGRADING.md", "from 1.25.x through 1.35.4 to 1.36.1 is intentionally"},
		},
		{
			path:     "docs/resman.8",
			required: []string{"/usr/share/doc/resman/UPGRADING.md", "Upgrade from 1.25.x through 1.35.4 to 1.36.1"},
		},
		{
			path:     "packaging/deb/control.in",
			required: []string{"Read /usr/share/doc/resman/UPGRADING.md before upgrading from 1.25.x through 1.35.4 to 1.36.1"},
		},
		{
