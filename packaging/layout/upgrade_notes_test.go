...
func TestUpgradeGuideCoversBreakingContracts(t *testing.T) {
	...
	required := []string{
		"# Upgrading from ResMan 1.25.x through 1.34.1 to ResMan 1.35.2",
...
func TestUpgradeGuideIsReferencedAndPackaged(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path:     "README.md",
			required: []string{"docs/UPGRADING.md", "from 1.25.x through 1.34.1 to 1.35.2 is intentionally"},
		},
		{
			path:     "docs/resman.8",
			required: []string{"/usr/share/doc/resman/UPGRADING.md", "Upgrade from 1.25.x through 1.34.1 to 1.35.2"},
		},
		{
			path:     "packaging/deb/control.in",
			required: []string{"Read /usr/share/doc/resman/UPGRADING.md before upgrading from 1.25.x through 1.34.1 to 1.35.2"},
		},
		{
