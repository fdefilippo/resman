...
func TestUpgradeGuideCoversBreakingContracts(t *testing.T) {
	...
	required := []string{
		"# Upgrading from ResMan 1.25.x to ResMan 1.30.4",
...
func TestUpgradeGuideIsReferencedAndPackaged(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path:     "README.md",
			required: []string{"docs/UPGRADING.md", "Upgrading from 1.25.x to 1.30.4 is intentionally breaking"},
		},
		{
			path:     "docs/resman.8",
			required: []string{"/usr/share/doc/resman/UPGRADING.md", "Upgrade from 1.25.x to 1.30.4"},
		},
		{
			path:     "packaging/deb/control.in",
			required: []string{"Read /usr/share/doc/resman/UPGRADING.md before upgrading from 1.25.x to 1.30.4"},
		},
		{
