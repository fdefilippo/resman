...
* Tue Sep 15 2026 Francesco Defilippo <francesco@defilippo.org> - 1.38.0-5
- Rebuild the selector-replacement correction with the explicit EL9 identity

* Tue Sep 15 2026 Francesco Defilippo <francesco@defilippo.org> - 1.38.0-4
- Replace reduced weighted-I/O device sets through one ordered D-Bus mutation

* Tue Sep 15 2026 Francesco Defilippo <francesco@defilippo.org> - 1.38.0-3
- Preserve owned weighted-I/O leases across atomic device-selector changes
...
* Fri Sep 11 2026 Francesco Defilippo <francesco@defilippo.org> - 1.36.4-1
- Accept direct and scaled BFQ kernel representations for systemd I/O weights.
- Make the startup I/O probe distinguish every supported representation from baseline.

* Fri Sep 11 2026 Francesco Defilippo <francesco@defilippo.org> - 1.36.3-1
- Materialize the I/O controller before validating io.max on systemd 239.
- Preserve fail-closed startup when an enabled I/O interface is genuinely unavailable.
...
* Tue Sep 08 2026 Francesco Defilippo <francesco@defilippo.org> - 1.35.3-3
- Prevent the readiness-aware workload launcher from starting workers twice.
- Preserve the failed 1.35.3-2 field evidence instead of rebuilding its package identity.
...
* Tue Sep 08 2026 Francesco Defilippo <francesco@defilippo.org> - 1.35.0-6
- Remove retired cgroup ownership keys from the retained PSI real-kernel fixture.
- Preserve the failed release 5 matrix evidence instead of rebuilding its identity.
...
- Remove migration-only configuration, telemetry, MCP fields and functional claims.
...
* Mon Sep 07 2026 Francesco Defilippo <francesco@defilippo.org> - 1.33.0-6
- Reject package builds that cannot expand the required systemd scriptlet macros
...
- RELOAD: expose bounded outcomes, suppress unchanged retry noise and report all removed keys
- SERVICE: separate the daemon shutdown deadline from MCP shutdown
- PACKAGING: remove the invalid logrotate postrotate hook
- CI: parse workflow failure controls structurally and fail closed
