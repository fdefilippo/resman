...
prepared_image=${RESMAN_IO_EFFECT_PREPARED_IMAGE:-/var/lib/libvirt/images/resman-iow-effect-prepared-ol9u8-rhck-5.14.0-687.46.1.el9_8.x86_64.qcow2}
...
[[ $package_identity == resman-1.38.0-7.el9.x86_64 ]] \
...
prepared_image=$("$script_dir/prepare-base.sh" "$base_image" "$prepared_image" \
	2>>"$evidence_dir/prepared-base.log")
prepared_manifest=$prepared_image.manifest
[[ -f $prepared_image && ! -L $prepared_image && -f $prepared_manifest && ! -L $prepared_manifest ]] \
	|| { echo "prepared RHCK image is incomplete" >&2; exit 77; }
prepared_sha=$(sha256sum "$prepared_image" | awk '{print $1}')
printf 'prepared_image=%s\nprepared_sha256=%s\n' "$prepared_image" "$prepared_sha" \
	>>"$evidence_dir/environment.txt"
install -m 0600 "$prepared_manifest" "$evidence_dir/prepared-base.manifest"

install -d -o qemu -g qemu -m 0710 "$work_dir"
qemu-img create -q -f qcow2 -F qcow2 -b "$prepared_image" "$overlay"
...
LIBGUESTFS_BACKEND=direct virt-customize -q -a "$overlay" \
	--ssh-inject "root:file:$private_key.pub" \
	--run-command 'restorecon -RF /root/.ssh'
...
run_guest_campaign() {
	local profile=$1 guest_evidence=$2 host_evidence=$3 log_file=$4 validation_file=$5
	set +e
	guest "python3 /root/resman-iow-effect/guest_effect.py --bundle /root/resman-iow-effect --evidence '$guest_evidence' --profile '$profile' --run-id '$run_id' --revision '$source_revision' --source-tree '$source_tree' --qualification-revision '$qualification_revision' --qualification-tree '$qualification_tree' --package /root/resman-iow-effect/package.rpm --device /dev/disk/by-id/virtio-resmaneffecta --device /dev/disk/by-id/virtio-resmaneffectb" \
		>"$log_file" 2>&1
	guest_status=$?
	set -e
	mkdir -p "$host_evidence"
	if guest "test -d '$guest_evidence'"; then
		guest "tar -C '$guest_evidence' -cf - ." \
			| tar --no-same-owner -C "$host_evidence" -xf -
	else
		detail="$profile campaign exited before producing evidence"
		[[ $guest_status -ne 0 ]] || guest_status=1
		return "$guest_status"
	fi
	if [[ $guest_status -ne 0 ]]; then
		detail="$profile campaign failed after producing checkpoint evidence"
		return "$guest_status"
	fi
	(
		cd "$host_evidence"
		manifest_tmp=$evidence_dir/guest-$profile-SHA256SUMS.tmp
