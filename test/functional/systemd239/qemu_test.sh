...
grep -q "if guest 'test -d /root/resman-systemd239/evidence'" "$script_dir/qemu-host.sh"
grep -q 'guest package gate exited before producing evidence' "$script_dir/qemu-host.sh"
grep -q 'python3 /root/resman-systemd239/el8_package.py' "$script_dir/qemu-host.sh"
grep -q 'CPUQuotaPerSecUSec=infinity' "$script_dir/qemu-host.sh"
...
grep -q 'test/functional/real-kernel/native_gate.py)' "$script_dir/remote-qemu.sh"
grep -q 'test/functional/real-kernel/native_package.py)' "$script_dir/remote-qemu.sh"
grep -q 'test/functional/real-kernel/native-workload.py)' "$script_dir/remote-qemu.sh"
